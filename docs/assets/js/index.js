...
var x = setInterval(
	function() {
		...
		document.getElementById("gecount").innerHTML = 'Outbreak Global Event begins in ' + days + ":" + ("0" + hours).slice(-2) + ":" + ("0" + minutes).slice(-2) + ":" + ("0" + seconds).slice(-2);

		// If the count down is over, write some text 
		if (distance < 0) {
			clearInterval(x);
			document.getElementById("gecount").innerHTML = "Outbreak Global Event has begun";
		}
	}
, 1000);
